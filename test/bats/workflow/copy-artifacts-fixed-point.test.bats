# End-to-end cover for the fixed-point step of rainix-copy-artifacts, driven
# from the step's own `run:` text so the wiring between the workflow input, the
# pinned flake refs and `rainix-static codegen-fixed-point` is exercised rather
# than assumed. The loop's own semantics are unit-tested in Rust.
#
# `$n` and `${{ … }}` inside single quotes are deliberate and must NOT expand
# here: the first belongs to the codegen script this file writes, the second to
# GitHub's own expansion of the step.
# shellcheck disable=SC2016

setup() {
  repo_root="$BATS_TEST_DIRNAME/../../.."
  workflow="$repo_root/.github/workflows/rainix-copy-artifacts.yaml"
  step="$(yq -r '.jobs["copy-artifacts"].steps[]
    | select(.name == "Regenerate committed artifacts to a fixed point")
    | .run' "$workflow")"
  # Read from the workflow rather than restated, so the bound these tests
  # exercise is the one a consumer gets when they pass no `with:` at all.
  ...
  # `nix` and `forge` are the only two binaries the step reaches for. Stubbing
  # them as files on PATH — not shell functions — is what lets the real
  # `rainix-static` reach them: it runs the pipeline in a bash it spawns itself.
  mkdir -p "$work/bin"
  cat >"$work/bin/nix" <<'STUB'
#!/usr/bin/env bash
# Anything but the two entry points the step is written against is an error,
# never a silent pass: a renamed devshell, a dropped `--`, or an unpinned ref
# must not still look like a working step.
case "$1" in
  run)
    [ "$2" = "github:rainlanguage/rainix/$RAINIX_TEST_SHA#rainix-static" ] ||
...
# The step as GitHub runs it: expressions expanded, then the shipped text.
run_step() {
  local script
  script="$(printf '%s\n' "$step" |
    sed -e "s|\${{ env.RAINIX_SHA }}|$RAINIX_TEST_SHA|g" \
      -e "s|\${{ inputs.max-codegen-passes }}|${1:-$default_passes}|g")"
  ...
  case "$script" in
  *'${{'*)
    echo "unexpanded expression left in the step: $script"
    return 90
    ;;
  esac
  (cd "$consumer" && bash -c "$script")
}
...
@test "every flake ref in the step is pinned to the workflow's sha" {
  [[ "$step" != *'github:rainlanguage/rainix#'* ]]
  [[ "$step" != *'github:rainlanguage/rainix/main'* ]]
  # One per command the step runs: build-meta.sh, Build.sol, build, copy, fmt,
  # and the rainix-static that loops them.
  [ "$(grep -cF 'github:rainlanguage/rainix/${{ env.RAINIX_SHA }}' <<<"$step")" -eq 6 ]
}
