      ...
      let
        ...
        pre-commit = git-hooks-nix.lib.${system}.run {
          ...
          hooks = {
            ...
            # TypeScript/JavaScript - conditional, only runs if config exists
            eslint-conditional = {
              enable = true;
              name = "eslint";
              entry = "${pkgs.writeShellScript "eslint-conditional" ''
                set -euxo pipefail

                find_eslint_config() {
                  local dir="$1"
                  while [ "$dir" != "/" ] && [ "$dir" != "." ]; do
                    if [ -f "$dir/eslint.config.js" ] || [ -f "$dir/eslint.config.mjs" ] || [ -f "$dir/eslint.config.cjs" ]; then
                      echo "$dir"
                      return 0
                    fi
                    dir="$(dirname "$dir")"
                ...
                for file in "$@"; do
                  ...
                  config_dir="$(find_eslint_config "$dir")" || continue
                  config_files["$config_dir"]+=" $file"
                done

                for config_dir in "''${!config_files[@]}"; do
                  files="''${config_files[$config_dir]}"
                  (cd "$config_dir" && ${pkgs.eslint}/bin/eslint --fix $files)
                done
              ''}";
            ...
            prettier-conditional = {
              enable = true;
              name = "prettier";
              entry = "${pkgs.writeShellScript "prettier-conditional" ''
                set -euxo pipefail

                find_prettier_root() {
                  local dir="$1"
                  while [ "$dir" != "/" ] && [ "$dir" != "." ]; do
                    if [ -f "$dir/.prettierrc" ] || [ -f "$dir/.prettierrc.json" ] || [ -f "$dir/.prettierrc.js" ] || [ -f "$dir/prettier.config.js" ] || [ -f "$dir/package.json" ]; then
                      echo "$dir"
                      return 0
                    fi
                    dir="$(dirname "$dir")"
                ...
                for file in "$@"; do
                  dir="$(dirname "$(realpath "$file")")"
                  config_dir="$(find_prettier_root "$dir")" || continue
                  config_files["$config_dir"]+=" $file"
                done

                for config_dir in "''${!config_files[@]}"; do
                  files="''${config_files[$config_dir]}"
                  (cd "$config_dir" && npx prettier --ignore-unknown --list-different --write $files)
                done
              ''}";
              files =
                "\\.(js|mjs|cjs|ts|tsx|json|css|scss|md|yaml|yml|svelte|html)$";
              pass_filenames = true;
            };
