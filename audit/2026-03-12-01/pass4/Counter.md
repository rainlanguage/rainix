...
`Counter.sol` uses `pragma solidity ^0.8.25` while `Counter.t.sol` uses
`pragma solidity ^0.8.13`. The source contract requires a newer compiler than
the test file. While Foundry resolves this by using the highest required
version, the inconsistency suggests the test pragma was not updated when the
source pragma was bumped.
